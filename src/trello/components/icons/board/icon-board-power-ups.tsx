...
type IconBoardPowerUpsProps = {
  className?: string;
  active?: boolean;
};

export const IconBoardPowerUps: React.FC<IconBoardPowerUpsProps> = ({
  className = 'w-4 h-4',
  active = false,
}) => {
  return (
    <svg
      className={`${className} ${active ? 'text-black' : 'text-white'} transition-colors hover:text-gray-200`}
      viewBox="0 0 16 16"
      fill="none"
    ...
    >
      <path
        ...
        clipRule="evenodd"
      />
      <path fill="currentColor" d="M12.5 4.625a1.125 1.125 0 1 1-2.25 0 1.125 1.125 0 0 1 2.25 0" />
    </svg>
  );
};
